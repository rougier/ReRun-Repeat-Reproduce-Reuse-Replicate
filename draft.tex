...
\author{
  ...
  \begin{minipage}{0.8\textwidth}
    \begin{center}
      ...
      \href{mailto:fabien@benureau.com}{fabien@benureau.com}
    \end{center}
  \end{minipage}
}
\date{}
...
Replicability\footnote{\emph{Reproducibility} and \emph{replicability} are employed differently by different authors and in different domains (see for instance the report from the  \cite{Schwalbe:2016}). Here, we place ourselves in the context of computational works, where data is produced by a program. In this paper, we call a result \emph{reproducible} if one can take the original source code, re-execute it and reobtain the original result. Conversely, a result is \emph{replicable} if one can create a code that matches the algorithmic descriptions given in the published article and reobtain the original result.} is a cornerstone of science.  If an experimental result cannot be
...
Published results should obviously come from versions of the code where every change and every file has been committed. This includes pre-processing, post-processing and plotting code. Plotting code may seem mundane, but it is as vulnerable as any other piece of the code to bugs and errors.
...
def compute_results(count, x0=0, step=1, seed=0):
    ...
    # Compute results
    walk = compute_walk(count=count, x0=x0, step=step, seed=seed)
    return {"data"      : walk,
...
if __name__ == "__main__":
    ...
    assert compute_walk(10, 0, 1, 42) == [1,0,-1,-2,-1,0,1,0,-1,-2]
...
Throughout the evolution of a small random walk example implemented in Python, we illustrated some of the issues that may plague scientific code. The code may be correct and of good quality and still possess many problems that reduce its contribution to the scientific discourse.
...
published. It's hardly the case today.\\
